...
## Orthofinder 通过比对蛋白质相似度鉴定同源基因

### 1. 先使用 Orthofinder 工具计算同源关系，然后使用脚本从中提取 1:1 的同源基因
...
### 2. 使用 R 脚本提取同源基因   
    a. 提取**单拷贝** 1:1 同源基因       
    Rscript scripts/one_to_one_orthologgene.R      
    a. 提取**成对**的 1:1 同源基因       
    Rscript scripts/one2one_Orthologgenes.R      



## Biomart下载同源基因，然后提取1:1同源基因


[脚本](比较两种同源基因.R) 中包含如何从biomart下载的结果中提取 1:1 同源基因的内容

```
# 有时候相同Symbol的基因可能没有被鉴定为1:1的同源基因，而是在1:many中，等其他情况，我们这里使用两种方法找同源基因，能够增加最终同源基因的书里那个
